...
def abre_dbf(subpasta,dbf_alvo,pasta_alvo):#devolve o arquivo como um objeto table do dbf
    ...
    return DBF(dbf_file, load=True, recfactory=dict) #atribui o DBF à variavel, a abre e retorna como chamado da função
...
def escreve_bd(cnx,dict_alvos,pasta_alvo):
    ...
    for pasta in dict_alvos:
        ...
        for arq in dict_alvos[pasta]:
            ...
            print("Tempo de loading dbf: ",time.time()-inicio)
            linhas,colunas = len(dbf_file),len(dbf_file.field_names)
            print("{} aberto:(Linhas:{} x Colunas: {})".format(arq, linhas, colunas))
            stmt = "INSERT INTO {} {} VALUES ({}{})".format(pasta, str(tuple(dbf_file.field_names)).replace("'", ""), "%s, "*(len(dbf_file.field_names)-1), "%s" )#
